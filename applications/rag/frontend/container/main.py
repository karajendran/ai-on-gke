...
# The standalone_query_generation_llm uses a temperature of 0.0 instead of 0.2 for the
...
chat_template_with_history = ChatPromptTemplate.from_messages(
    [
        ("system", "Always assist with care, respect, and truth. Respond with utmost utility yet securely. Avoid harmful, unethical, prejudiced, or negative content. Ensure replies promote fairness and positivity. You are a helpful AI bot. Your name is Gary."),
...
rag_llm_chain = LLMChain(llm=response_with_context_generation_llm, prompt=prompt_with_context_template)
...
def handlePrompt():
    ...
    try:
        log.info("enable_chat_history global var: ", enable_chat_history)
        prompt_with_history = user_prompt
        if enable_chat_history:
            log.info("fetching history")
            prompt_with_history = chain_with_history.invoke(
...
if __name__ == '__main__':
    ...
    init_chat_history()
    log.info("enable_chat_history env var: ", enable_chat_history)
